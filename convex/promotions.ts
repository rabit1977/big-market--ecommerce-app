...
export const runDailyRefreshes = mutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(
      `Bumping ${listingsToRefresh.length} listings for daily refresh...`,
    );
...
export const checkExpiringPromotions = mutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(
      `Sending expiry notifications for ${expiringListings.length} listings...`,
    );
...
export const checkExpiredPromotions = mutation({
  ...
  handler: async (ctx) => {
    ...
    // Find promoted listings that have already expired
    const expiredListings = await ctx.db
      .query('listings')
      .withIndex('by_promoted', (q) => q.eq('isPromoted', true))
      .filter((q) => q.lt(q.field('promotionExpiresAt'), now))
      .collect();

    console.log(
      `Handling expiration for ${expiredListings.length} promotions...`,
    );

    for (const listing of expiredListings) {
      // 1. Clear promotion status
      await ctx.db.patch(listing._id, {
        isPromoted: false,
        promotionTier: undefined,
        promotionExpiresAt: undefined,
        promotionPackageId: undefined,
      });
      ...
      await ctx.db.insert('notifications', {
        ...
        message: `Your promotion for "${listing.title}" has expired. Renew it now to regain your top position!`,
        link: `/listings/${listing._id}`,
        isRead: false,
        createdAt: now,
      });
    ...
    return { count: expiredListings.length };
  },
});
