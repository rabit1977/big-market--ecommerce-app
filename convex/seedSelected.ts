...
function getDynamicFields(path: string) {
  ...
  // Fashion/Apparel
  if (lowerPath.includes("apparel") || lowerPath.includes("clothing") || lowerPath.includes("shoes") || lowerPath.includes("jewelry")) {
    fields.push(COMMON_FIELDS.BRAND);
    if (!lowerPath.includes("accessories")) {
      fields.push(COMMON_FIELDS.SIZE, COMMON_FIELDS.COLOR);
    }
    fields.push(COMMON_FIELDS.GENDER);
  } 
  // Pets
  else if (lowerPath.includes("animals") || lowerPath.includes("pet supplies")) {
    if (lowerPath.includes("dog") || lowerPath.includes("cat")) {
       fields.push({ label: "Brand", type: "text", key: "brand", placeholder: "e.g. Royal Canin" });
    }
    if (lowerPath.includes("apparel")) {
       fields.push(COMMON_FIELDS.SIZE, COMMON_FIELDS.COLOR);
    }
  }
  else {
...
const TAXONOMY_DATA = `1 - Animals & Pet Supplies
...
5411 - Apparel & Accessories > Clothing > Baby One-Pieces
...
5564 - Apparel & Accessories > Clothing > Uniforms > Soccer Uniforms
...
181 - Apparel & Accessories > Clothing Accessories > Wigs
...
export const seedSelectedGoogleTaxonomy = action({
  args: {},
  handler: async (ctx) => {
    const lines = TAXONOMY_DATA.split("\n");
    ...
    for (const line of lines) {
      ...
      // Keep slug unique with ID since same names exist in different branches
      slug = `${slug}-${id}`;
    ...
    console.log(`Parsed ${categories.length} categories. Purging current categories...`);
    await ctx.runAction(api.seedGoogle.clearCategories);

    console.log("Seeding new categories...");
    await ctx.runMutation(api.seedGoogle.insertCategoryChunk, { chunk: categories });
...
export const getCount = query({
  args: {},
  handler: async (ctx) => {
    // We limit to 4000 for summary count to avoid read limit crashes
    const cats = await ctx.db.query("categories").take(4000);
    return cats.length;
  }
});
