    ...
    categories.map(async (cat) => {
      ...
      if (isSubCategory) {
        count = (
          await ctx.db
            .query('listings')
            .withIndex('by_status_subCategory', (q: any) =>
              q.eq('status', 'ACTIVE').eq('subCategory', cat.slug),
            )
            .collect()
        ).length;
      } else {
        count = (
          await ctx.db
            ...
            .withIndex('by_status_category', (q: any) =>
              q.eq('status', 'ACTIVE').eq('category', cat.slug),
            )
            .collect()
        ).length;
      }
...
export const getWithCounts = query({
  ...
  handler: async (ctx) => {
    ...
    // Fetch all active listings once to calculate counts efficiently
    ...
    for (const listing of activeListings) {
      if (listing.category) {
        parentCounts.set(
          listing.category,
          (parentCounts.get(listing.category) || 0) + 1,
        );
      }
      if (listing.subCategory) {
        subCounts.set(
          listing.subCategory,
          (subCounts.get(listing.subCategory) || 0) + 1,
        );
      }
    }
