...
export default function EditProfilePage() {
  ...
  const handleUpdateProfile = async (values: EditProfileFormValues) => {
    ...
    try {
      ...
      toast.success(
        isMk ? 'Профилот е успешно ажуриран' : 'Profile updated successfully',
      );
      ...
      toast.error(
        isMk ? 'Неуспешно ажурирање на профилот' : 'Failed to update profile',
      );
    } finally {
  ...
  if (user === null) {
    return (
      <div className='p-20 text-center'>
        {isMk ? 'Корисникот не е пронајден' : 'User not found'}
      </div>
    );
  }
    ...
    <div className='min-h-screen pt-4 md:pt-6 pb-8 bg-background'>
      <div className='container max-w-4xl mx-auto px-3 md:px-4'>
        ...
        <div className='mb-5 md:mb-8'>
          <Link
            href='/account'
            className='text-xs md:text-sm text-muted-foreground flex items-center gap-1 hover:text-foreground mb-2 transition-colors'
          >
            <ArrowLeft className='h-3.5 w-3.5' />{' '}
            {isMk ? 'Назад' : 'Back to Account'}
          </Link>
          ...
          <p className='text-muted-foreground text-xs md:text-sm font-medium'>
            {isMk
              ? 'Управувајте со вашиот јавен профил и преференции'
