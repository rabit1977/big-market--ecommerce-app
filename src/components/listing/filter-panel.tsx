...
export function FilterPanel({
  ...
  const [dynamicFilters, setDynamicFilters] = useState<Record<string, any>>(
    () => {
      ...
      try {
        return JSON.parse(initialFilters.dynamicFilters);
      } catch {
        return {};
      }
    },
  ...
  const [region, setRegion] = useState<string | undefined>(() => {
    if (initialFilters?.city) {
      for (const [r, muns] of Object.entries(MK_LOCATIONS)) {
        if (r === initialFilters.city || muns.includes(initialFilters.city))
          return r;
      }
    }
    return undefined;
  ...
  useEffect(() => {
    ...
    if (initialFilters.city && initialFilters.city !== 'all') {
      ...
      Object.entries(MK_LOCATIONS).forEach(([region, municipalities]) => {
        if (
          municipalities.some(
            (m) => m.toLowerCase() === initialFilters.city?.toLowerCase(),
          )
        ) {
    ...
    try {
      setDynamicFilters(
        initialFilters.dynamicFilters
          ? JSON.parse(initialFilters.dynamicFilters)
          : {},
      );
    } catch {
      setDynamicFilters({});
  ...
  const subCategories = useMemo(
    () =>
      selectedMainCategory
        ? categories.filter((c) => c.parentId === selectedMainCategory._id)
        : [],
  ...
  const selectedSubCategory = useMemo(() => {
    ...
    if (
      direct.parentId &&
      categories.find((c) => c._id === direct.parentId && !c.parentId)
    )
      return direct;
    ...
    if (direct.parentId) {
      ...
      if (
        parent?.parentId &&
        categories.find((c) => c._id === parent.parentId && !c.parentId)
      )
        return parent;
    }
    return direct;
  ...
  const subSubCategories = useMemo(
    () =>
      selectedSubCategory
        ? categories.filter((c) => c.parentId === selectedSubCategory._id)
        : [],
  ...
  const filterableTemplateFields = useMemo(() => {
    ...
    return template.fields.filter((f: any) => {
      if (
        ['title', 'description', 'price', 'images', 'condition'].includes(f.key)
      )
        return false;
      return (
        f.type === 'select' && f.options?.length > 1 && f.options.length <= 20
      );
    });
  }, [template]);
  ...
  const toggleDynamicArrayFilter = useCallback(
  ...
  const activeFilterCount = useMemo(
    () =>
      Object.keys(filters).filter(
        (k) => k !== 'sortBy' && filters[k as keyof FilterState],
      ).length,
    [filters],
  );
    ...
    <div className='space-y-3'>
      <Card className='p-3 space-y-3 rounded-lg border-none shadow-none bg-card bm-interactive'>
        ...
        <div className='space-y-1 border-b border-border pb-2'>
          <Label
            htmlFor={`${idPrefix}-listingNumber`}
        ...
        <div className='space-y-2 pb-2 border-b border-border'>
            ...
            <div className='space-y-1'>
              ...
              >
                <SelectTrigger
                  ...
                  className='h-8 text-xs rounded-md bm-interactive'
                >
          ...
          {subSubCategories.length > 0 && (
            <div className='space-y-1'>
              <Label
                htmlFor={`${idPrefix}-subsubcategory`}
                className='text-[10px] uppercase text-muted-foreground font-medium'
              >
                {t('type')}
              </Label>
              <Select
                value={
                  selectedSubCategory &&
                  subSubCategories.some((c) => c.slug === filters.subCategory)
                    ? filters.subCategory
                    : 'all'
                }
                        ...
                        {field.options.map((opt: string) => (
                          ...
                          >
                            <Checkbox
                              ...
                              onCheckedChange={() =>
                                toggleDynamicArrayFilter(field.key, opt)
                              }
        ...
        <div className='space-y-1.5 pb-2 border-b border-border'>
          ...
          <Select
            ...
            onValueChange={(val) =>
              updateFilter('dateRange', val === 'all' ? undefined : val)
            }
          >
            <SelectTrigger
              id={`${idPrefix}-date`}
        ...
        <div className='space-y-3'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
            <div className='space-y-1'>
              <Label
                htmlFor={`${idPrefix}-adType`}
                className='text-[10px] uppercase text-muted-foreground font-medium'
              >
                {t('ad_type') || 'Тип на оглас'}
              </Label>
              <Select
                value={filters.adType || 'all'}
                onValueChange={(val) =>
                  updateFilter('adType', val === 'all' ? undefined : val)
                }
              >
                <SelectTrigger
                  id={`${idPrefix}-adType`}
            ...
            <div className='space-y-1'>
              <Label
                htmlFor={`${idPrefix}-userType`}
                className='text-[10px] uppercase text-muted-foreground font-medium'
              >
                {t('user_type') || 'Продавач'}
              </Label>
              <Select
                value={filters.userType || 'all'}
                onValueChange={(val) =>
                  updateFilter('userType', val === 'all' ? undefined : val)
                }
              >
                <SelectTrigger
            ...
            <div className='flex flex-wrap gap-1.5'>
              {filters.category && filters.category !== 'all' && (
                <FilterBadge
                  ...
                  onRemove={() => handleMainCategoryChange('all')}
                />
              )}
              {filters.subCategory && filters.subCategory !== 'all' && (
                <FilterBadge
                  ...
                  onRemove={() => handleSubCategoryChange('all')}
                />
              )}
              {filters.city && filters.city !== 'all' && (
                <FilterBadge
                  label={getLocationLabel(filters.city)}
                  onRemove={() => updateFilter('city', undefined)}
                />
              )}
              {Object.entries(dynamicFilters).map(([key, value]) => {
                const fieldLabel =
                  template?.fields?.find((f: any) => f.key === key)?.label ??
                  key;
                return (
                  <FilterBadge
                    key={key}
                    label={`${fieldLabel}: ${value}`}
                    onRemove={() => updateDynamicFilter(key, undefined)}
                  />
                );
              })}
              {filters.condition && filters.condition !== 'all' && (
                <FilterBadge
                  ...
                  onRemove={() => updateFilter('condition', undefined)}
                />
              )}
              ...
              {filters.adType && (
                <FilterBadge
                  ...
                  onRemove={() => updateFilter('adType', undefined)}
                />
              )}
              {filters.userType && (
                <FilterBadge
                  label={
                    filters.userType === 'PRIVATE'
                      ? t('private') || 'Приватен'
                      : t('company') || 'Компанија'
                  }
                  onRemove={() => updateFilter('userType', undefined)}
                />
              )}
              {filters.isTradePossible && (
                <FilterBadge
                  label={t('trade_possible') || 'Замена'}
                  onRemove={() => updateFilter('isTradePossible', undefined)}
                />
              )}
              {filters.hasShipping && (
                <FilterBadge
                  label={t('has_shipping') || 'Испорака'}
                  onRemove={() => updateFilter('hasShipping', undefined)}
                />
              )}
              {filters.isVatIncluded && (
                <FilterBadge
                  label={t('is_vat_included') || 'Со ДДВ'}
                  onRemove={() => updateFilter('isVatIncluded', undefined)}
                />
              )}
              {filters.isAffordable && (
                <FilterBadge
                  label={t('is_affordable') || 'Поволно'}
                  onRemove={() => updateFilter('isAffordable', undefined)}
                />
              )}
...
function FilterBadge({
  ...
  return (
    <Badge
      variant='secondary'
      className='gap-1 text-[10px] h-6 px-2 rounded-md font-medium border border-border'
    >
      {label}
      <X
