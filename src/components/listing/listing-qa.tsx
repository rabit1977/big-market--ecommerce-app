    ...
    <div className="space-y-6 pt-6 mt-8 border-t">
       ...
       {!isOwner ? (
         <div className="bg-muted/50 p-4 md:p-6 rounded-2xl border mb-8">
           <h3 className="font-bold mb-3 text-sm">{t('ask_seller')}</h3>
           {session?.user ? (
             ...
             <div className="text-sm text-muted-foreground bg-background p-4 rounded-xl text-center border">
               {t('login_to_ask')} <a href="/login" className="text-primary font-bold hover:underline">{t('log_in')}</a> {t('login_suffix')}
             </div>
           )}
         </div>
       ) : (
       ...
       <div className="space-y-6">
         {questions.length > 0 ? (
           <>
             {(showAll ? questions : questions.slice(0, 2)).map((q) => (
                         ...
                         <div className="flex-1 bg-background p-3 rounded-xl border shadow-sm">
                           <div className="flex justify-between items-center mb-1">
                             <div className="flex items-center gap-1.5">
                               ...
                               {(a.isOfficial || a.userId === sellerId) && (
                                  <span className="bg-primary/10 text-primary text-[10px] uppercase font-black px-1.5 py-0.5 rounded-sm">{t('seller_badge')}</span>
                               )}
                             </div>
                 ...
                 {replyingTo === (q.id || q._id) && (
                   <div className="ml-4 md:ml-12 mt-2 bg-background p-3 rounded-xl border flex flex-col gap-2">
                     ...
                     <div className="flex justify-end gap-2">
                       <Button variant="ghost" size="sm" onClick={() => { setReplyingTo(null); setReplyText(''); }}>{t('cancel')}</Button>
                       <Button size="sm" onClick={() => handleReplySubmit(q.id || q._id)} disabled={isSubmitting || !replyText.trim()}>{t('submit_answer')}</Button>
                     </div>
                   </div>
                 )}
               </div>
             ...
             {!showAll && questions.length > 2 && (
               <Button 
                 variant="outline" 
                 className="w-full h-11 rounded-xl font-bold uppercase tracking-widest text-[11px] border-dashed"
                 onClick={() => setShowAll(true)}
               >
                 {t('show_more')} ({questions.length - 2})
               </Button>
             )}
           </>
         ) : (
           <div className="text-center py-10 bg-muted/30 rounded-2xl border border-dashed text-muted-foreground">
             ...
             <p className="text-sm font-medium">{t('no_questions')}</p>
             {!isOwner && <p className="text-xs mt-1">{t('be_first')}</p>}
           </div>
         )}
       </div>
    </div>
