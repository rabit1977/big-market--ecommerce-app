...
export function SupportChatWidget() {
  ...
  const totalUnreadForAdmin =
    useQuery(api.messages.getTotalSupportUnread, isAdmin ? {} : 'skip') ?? 0;
  ...
  useEffect(() => {
    ...
    if (isAdmin && activeChatUser && adminActiveMessages.length > 0) {
      ...
      if (last.senderId !== ADMIN_ID && !last.read)
        markSafe(activeChatUser, ADMIN_ID);
    }
  }, [
    isOpen,
    userId,
                    ...
                    <div className='flex items-center gap-1.5'>
                      <div
                        className={cn(
                          ...
                          (!isAdmin && onlineStatusMap[ADMIN_ID]) ||
                            (isAdmin &&
                              activeChatUser &&
                              onlineStatusMap[activeChatUser])
                ...
                <div className='flex-1 flex flex-col items-center justify-center p-8 text-center space-y-4'>
                  ...
                  <div className='space-y-1'>
                    ...
                    <p className='text-sm text-muted-foreground'>
                      Log in to contact our support team.
                    </p>
                  </div>
                </div>
              ) : isAdmin && !activeChatUser ? (
                ...
                <div className='flex-1 flex flex-col overflow-hidden'>
                  ...
                  <div className='px-4 py-2 bg-background dark:bg-slate-800/50 border-b border-border'>
                    <p className='text-[10px] text-muted-foreground font-bold uppercase tracking-widest text-center'>
                      Select a chat to view or reply
                    </p>
                  </div>
                    ...
                    <div className='space-y-4'>
                      {!isAdmin && (
                        <div className='bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl rounded-bl-none px-4 py-3 text-sm max-w-[85%] shadow-sm'>
                          <p className='font-bold text-slate-900 dark:text-slate-100 mb-1 text-xs'>
                            PazarPlus Support
                          </p>
                          <p className='!text-slate-900 dark:!text-slate-100'>
                            Hi! How can we help you today? We're usually online
        ...
        <AnimatePresence mode='wait'>
          {!isOpen && (
            ...
            >
              ...
              {showUnreadBadge && (
                <span
                  className='absolute -top-1 -right-1 h-4 w-4 bg-red-600 rounded-full border-2 border-background flex items-center justify-center text-[9px] font-bold text-white shadow-sm animate-pulse'
                  aria-label='Unread messages'
                >
                  {isAdmin
                    ? totalUnreadForAdmin > 9
                      ? '9+'
                      : totalUnreadForAdmin
                    : 1}
                </span>
