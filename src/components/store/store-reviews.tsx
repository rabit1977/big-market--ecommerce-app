...
export function StoreReviews({ sellerId }: { sellerId: string }) {
  ...
  return (
    <div id='reviews' className='space-y-6 pt-12 scroll-mt-24'>
      <div className='flex items-center justify-between border-b pb-4'>
        <h2 className='text-xl md:text-2xl font-bold uppercase tracking-tight'>
          {t('title')}
        </h2>
        <span className='bg-primary hover:bg-primary/90 text-primary-foreground font-black text-xs px-3 py-1 rounded-full'>
      ...
      {reviews.length > 0 ? (
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6'>
          {reviews.map((review) => (
            <div
              key={review._id}
              className='p-5 md:p-6 rounded-2xl border bg-card/50 shadow-sm flex flex-col gap-4 transition-colors hover:bg-card'
            >
              ...
              <div className='mt-2 pt-4 border-t border-dashed text-xs text-muted-foreground/70 font-medium'>
                {t('reviewed_item')}{' '}
                <span className='text-muted-foreground line-clamp-1 mt-0.5 italic'>
                  {review.listingTitle}
                </span>
              </div>
            </div>
        ...
        <div className='text-center py-16 bg-muted/40 rounded-3xl border border-dashed border-border/60'>
          ...
          <h3 className='text-base font-bold text-foreground mb-1 uppercase tracking-tight'>
            {t('no_reviews_title')}
          </h3>
