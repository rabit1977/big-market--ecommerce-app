...
export default function ComparePage() {
  ...
  if (!mounted) {
    return <div className='min-h-screen pt-4 md:pt-6 pb-20 bg-muted/10'></div>;
  }
      ...
      <div className='min-h-screen pt-4 md:pt-6 pb-20 bg-muted/10'>
        <div className='container-wide mx-auto px-4 flex flex-col items-center justify-center min-h-[60vh]'>
          ...
          <p className='text-muted-foreground mt-2 max-w-sm text-center text-sm mb-6'>
            Go back and select up to 3 listings using the compare icon on the
            listing cards to see them side-by-side.
          </p>
        ...
        <div className='glass-panel overflow-hidden rounded-2xl border border-border/50 bg-card'>
          <ScrollArea className='w-full relative'>
            <table className='w-full text-left border-collapse min-w-[800px]'>
              <thead>
                <tr>
                  ...
                  {items.map((item: ListingWithRelations) => (
                    <th
                      key={item._id}
                      className='p-4 sm:p-6 w-[250px] sm:w-[300px] xl:w-[350px] border-r border-border/30 align-top relative group group-hover:bg-muted/10 transition-colors'
                    >
                      <Button
                        variant='destructive'
                        size='icon'
              ...
              <tbody>
                ...
                <tr className='border-t border-border/50 hover:bg-muted/20 transition-colors'>
                  <td className='p-4 sm:p-5 text-sm font-semibold text-muted-foreground border-r border-border/30 bg-muted/10 shrink-0'>
                    Status
                  </td>
                  {items.map((item: ListingWithRelations) => (
                    <td
                      key={item._id}
                      className='p-4 sm:p-5 text-sm font-medium border-r border-border/30'
                    >
                      <span
                        className={
                          item.status === 'ACTIVE'
                            ? 'text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md'
                            : 'text-orange-500 bg-orange-500/10 px-2 py-0.5 rounded-md'
                        }
                      >
                        {item.status}
                      </span>
                ...
                <tr className='border-t border-border/50 hover:bg-muted/20 transition-colors'>
                  ...
                  {items.map((item: ListingWithRelations) => (
                    <td
                      key={item._id}
                      className='p-4 sm:p-5 text-sm border-r border-border/30 font-medium capitalize'
                    >
                      {item.category}{' '}
                ...
                <tr className='border-t border-border/50 hover:bg-muted/20 transition-colors'>
                  ...
                  {items.map((item: ListingWithRelations) => (
                    <td
                      key={item._id}
                      className='p-4 sm:p-5 text-sm border-r border-border/30 font-medium'
                    >
                      {item.city}{' '}
                      {(item as any).region ? `, ${(item as any).region}` : ''}
                    </td>
                  ))}
                </tr>
                <tr className='border-t border-border/50 hover:bg-muted/20 transition-colors'>
                  ...
                  <>
                    <tr>
                      <td
                        ...
                        className='p-4 sm:p-6 bg-muted/30 border-y border-border/50'
                      >
                        <h4 className='font-bold text-sm tracking-widest text-foreground uppercase'>
                          Specifications
                        </h4>
                      </td>
                    </tr>
                    {allSpecKeys.map((key: string) => (
                      ...
                      >
                        ...
                        {items.map((item: ListingWithRelations) => (
                          <td
                            key={`${item._id}-${key}`}
                            className='p-3 sm:p-4 text-sm border-r border-border/30 font-medium'
                          >
                            {item.specifications &&
                            item.specifications[key] !== undefined &&
            ...
            <ScrollBar
              ...
              className='h-2.5 hover:h-3 transition-all cursor-grab active:cursor-grabbing'
            />
          </ScrollArea>
        </div>
      </div>
