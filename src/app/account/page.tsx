...
const AccountPage = async () => {
  ...
  const stats = {
    ...
    activeListings: userListings.filter((l: any) => l.status === 'ACTIVE')
      .length,
              ...
              <div className='flex-1 space-y-2 sm:space-y-3 min-w-0'>
                <div className='space-y-0.5'>
                  <div className='flex flex-wrap items-center justify-center sm:justify-start gap-2'>
                    ...
                    <Badge
                      variant='secondary'
                      className='bg-primary/10 text-primary hover:bg-primary/20 border-primary/20 font-bold px-2 py-0.5 rounded-xl text-[10px]'
                    >
                ...
                <div className='flex flex-wrap items-center justify-center sm:justify-start gap-3 sm:gap-5 text-[10px] sm:text-xs font-bold text-muted-foreground uppercase tracking-widest'>
                  ...
                  {stats.totalListings > 0 && (
                    <div className='flex items-center gap-1.5'>
                      ...
                      <span>
                        {stats.totalListings} {t('listings')}
                      </span>
                    </div>
                  )}
                </div>
          ...
          <div className='grid lg:grid-cols-12 gap-4 sm:gap-6 items-start'>
                ...
                <div className='p-6 sm:p-8 grid grid-cols-2 gap-6 sm:gap-8'>
                  {detailFields.map((item, i) => (
                    <div key={i} className='space-y-1 sm:space-y-2'>
                      ...
                      <p
                        className={`text-base sm:text-lg font-black text-foreground truncate ${item.capitalize ? 'capitalize' : ''}`}
                      >
                ...
                <Link href='/account/password' className='group'>
                  <div className='bg-card p-5 sm:p-6 rounded-2xl transition-all flex items-center gap-4 bm-interactive shadow-none'>
                    ...
                    <div className='min-w-0'>
                      <p className='font-black text-foreground text-xs sm:text-sm uppercase tracking-widest truncate'>
                        {t('security')}
                      </p>
                      <p className='text-[10px] sm:text-xs text-muted-foreground font-bold truncate'>
                        {t('update_password')}
                      </p>
                    </div>
                  </div>
            ...
            <div className='lg:col-span-4 space-y-4 sm:space-y-6'>
                    ...
                    <Link href={item.href} key={idx} className='block group'>
                      <div className='flex items-center justify-between p-3 rounded-xl hover:bg-muted transition-all duration-200'>
                        <div className='flex items-center gap-4'>
                          ...
                          <span className='font-black text-foreground text-xs sm:text-sm uppercase tracking-widest'>
                            {item.label}
                          </span>
                        </div>
              ...
              <div className='bg-primary/5 border border-primary/10 rounded-2xl md:rounded-2xl p-5 sm:p-6 relative overflow-hidden group'>
                ...
                <p className='text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-0.5 pointer-events-none'>
                  {t('account_balance')}
                </p>
                <p className='text-2xl sm:text-3xl font-black mb-5 sm:mb-6 text-foreground'>
                  0.00 MKD
                </p>
                <Button
                  asChild
                  className='w-full font-bold rounded-xl h-10 sm:h-11 shadow-sm text-sm'
                >
