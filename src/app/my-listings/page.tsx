...
export default async function MyListingsPage({
  ...
  if (!success) {
    ...
    return (
      <div className='p-8 text-center text-destructive font-bold text-sm uppercase tracking-wide'>
  ...
  const sortedListings = listings?.sort((a, b) => {
    const aPromoted =
      a.isPromoted && a.promotionExpiresAt && a.promotionExpiresAt > Date.now();
    const bPromoted =
      b.isPromoted && b.promotionExpiresAt && b.promotionExpiresAt > Date.now();
        ...
        <div className='animate-fade-in-up'>
          ...
          <div className='flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-6 md:mb-20 py-2 md:py-6 pb-6 border-b border-card-foreground/10'>
            <div className=''>
              <h2 className='text-2xl md:text-5xl font-black tracking-tighter uppercase leading-tight text-foreground'>
                ...
                <span className='text-primary'>
                  {t('page_title').split(' ').slice(1).join(' ') ||
                    t('page_title')}
                </span>
              </h2>
            ...
            <div className='flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto'>
              <Suspense
                fallback={
                  <div className='h-14 w-full sm:w-[350px] bg-muted/30 animate-pulse rounded-2xl' />
                }
              >
          ...
          {sortedListings && sortedListings.length > 0 ? (
            ...
            <div className='text-center py-20 md:py-40 bg-card/40 backdrop-blur-sm rounded-[3rem] border-1 border-card-foreground/10 bm-interactive shadow-none group'>
              ...
              <h3 className='text-3xl md:text-5xl font-black text-foreground uppercase tracking-tighter mb-6'>
                {t('empty_title')}
              </h3>
              ...
              <Button
                asChild
                size='lg'
                className='rounded-2xl font-black text-xs uppercase tracking-[0.2em] h-16 px-16 transition-all active:scale-95 shadow-2xl shadow-primary/20 border border-primary/20 bg-primary hover:bg-primary/95 text-white'
              >
