...
export default function ListingStatsPage({ params }: PageProps) {
  ...
  // Process data
  const dailyData =
    statsData?.dailyStats?.map(
      (d: { date: string; views: number; clicks: number }) => ({
        ...d,
  ...
  const { totalViews, totalClicks, totalFavorites } = statsData || {
    totalViews: 0,
    totalClicks: 0,
    totalFavorites: 0,
  };
            ...
            <Card className='rounded-[2rem] border border-border shadow-sm bg-card overflow-hidden h-full flex flex-col'>
              ...
              <CardContent className='p-4 md:p-6 flex-1 min-h-[350px]'>
                <div className='h-full w-full min-h-[300px]'>
                  {!statsData ? (
                    <div className='h-full w-full flex items-center justify-center text-muted-foreground text-xs font-bold uppercase tracking-wider animate-pulse'>
                      Loading analytics...
                    </div>
                  ) : dailyData.length === 0 ? (
                            ...
                            if (active && payload && payload.length) {
                              ...
                              return (
                                <div className='bg-popover/90 backdrop-blur-md text-popover-foreground text-xs rounded-2xl p-4 shadow-xl border border-border/50 min-w-[150px]'>
                                  <div className='font-black mb-3 uppercase tracking-wide text-foreground border-b border-border/50 pb-2'>
                                    {d.formattedDate}
                                  </div>
            ...
            <Card className='rounded-[2rem] border border-border shadow-sm bg-card overflow-hidden h-full flex flex-col max-h-[500px] lg:max-h-full'>
              ...
              <div className='overflow-y-auto custom-scrollbar flex-1 p-3 space-y-1'>
                {dailyData
                  .slice()
                  .reverse()
                  .filter((r: any) => r.views + r.clicks > 0)
                  .map((row: any, i: number) => (
                    <div
                      key={i}
                      className='flex items-center justify-between p-3.5 hover:bg-muted/50 rounded-2xl transition-all group border border-transparent hover:border-border/50 cursor-default'
                    >
