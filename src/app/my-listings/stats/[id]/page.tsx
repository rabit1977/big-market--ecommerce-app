...
export default function ListingStatsPage({ params }: PageProps) {
    ...
    const statsData = useQuery(api.analytics.getDetailedListingStats, { 
        listingId: id as Id<"listings">,
        days: 30
    });
    
    // Process data
    const dailyData = statsData?.dailyStats?.map((d: { date: string, views: number, clicks: number }) => ({
        ...d,
        formattedDate: format(new Date(d.date), 'dd MMM'),
        total: d.views + d.clicks
    })) || [];

    // Use server-aggregated tools if available, otherwise fallback (though server usually handles it now)
    const { totalViews, totalClicks, totalFavorites } = statsData || { totalViews: 0, totalClicks: 0, totalFavorites: 0 };
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-sm font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                           Listing Analysis <span className="w-1 h-1 bg-muted-foreground rounded-full" /> #{id.slice(-6)}
                        </p>
                    </div>
                    ...
                    <Card className="rounded-2xl md:rounded-[2rem] border border-border shadow-sm bg-card overflow-hidden group hover:border-orange-500/30 transition-all cursor-default relative">
                        ...
                        <CardContent className="p-3 md:p-6 flex flex-col justify-between h-full relative z-10 gap-2 md:gap-0">
                            <div className="flex items-center justify-between mb-1 md:mb-4">
                                ...
                                <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-muted-foreground bg-muted px-1.5 py-0.5 rounded-md md:rounded-lg">
                                    Traffic
                                </span>
                            </div>
                            <div>
                             ...
                             <div className="px-6 py-5 border-b border-border/50 bg-muted/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-4 bg-background/50 p-1.5 rounded-full border border-border/50 w-fit">
                                    ...
                                    <div className="flex items-center gap-2 px-3 border-l border-border/50">
                                        ...
                                        <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Contact Clicks</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="h-full w-full min-h-[300px]">
                                    {!statsData ? (
                                        ...
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={dailyData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }} barGap={2}>
                                                            ...
                                                            return (
                                                                <div className="bg-popover/90 backdrop-blur-md text-popover-foreground text-xs rounded-2xl p-4 shadow-xl border border-border/50 min-w-[150px]">
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        ...
                                                                        <div className="flex justify-between items-center gap-8">
                                                                            <span className="text-muted-foreground font-bold text-[10px] uppercase tracking-wider flex items-center gap-1.5">
                                                                                 <div className="w-1.5 h-1.5 rounded-full bg-blue-500" /> Clicks
                                                                            </span>
                    ...
                    <div className="lg:col-span-1">
                        <Card className="rounded-[2rem] border border-border shadow-sm bg-card overflow-hidden h-full flex flex-col max-h-[500px] lg:max-h-full">
                            ...
                            <div className="overflow-y-auto custom-scrollbar flex-1 p-3 space-y-1">
                                {dailyData.slice().reverse().filter((r: any) => (r.views + r.clicks) > 0).map((row: any, i: number) => (
                                    <div key={i} className="flex items-center justify-between p-3.5 hover:bg-muted/50 rounded-2xl transition-all group border border-transparent hover:border-border/50 cursor-default">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {row.clicks > 0 && (
                                                <div className="bg-blue-500/10 text-blue-600 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider">
                                                    {row.clicks} Leads
                                                </div>
                                            )}
                                            <div className="flex flex-col items-end">
                                                <span className="text-sm font-black text-foreground">{row.views}</span>
                                                <span className="text-[8px] text-muted-foreground font-bold uppercase tracking-widest">Views</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                {dailyData.every((d: any) => d.total === 0) && (
