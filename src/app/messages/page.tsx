...
export const metadata = {
  title: 'Messages | PazarPlus',
  description: 'Your conversations on PazarPlus',
};
...
export default async function MessagesPage({
  ...
  if (!session?.user) {
    redirect(
      `/auth/signin?callbackUrl=/messages${listingId ? `?listingId=${listingId}` : ''}`,
    );
  }
    ...
    if (!exists) {
      try {
        ...
        if (listing && listing.userId !== session.user.id) {
          ...
          const seller = await fetchQuery(api.users.getByExternalId, {
          ...
          const sellerDisplayName =
            ...
            (listing.city
              ? `${listing.city} Seller`
              : `User ${listing.userId.substring(0, 4)}`);
